...
1.  CMS 收集器对 CPU 资源非常敏感
    ...
    在 CMS 收集器中，GC 线程的数量是与 CPU 的数量相关。
    
    GC 线程数 = (CPU 个数 + 3) / 4；

    当 CPU 个数小于 2 时，CMS 收集器对用户程序的影响可能就会变得很大。

1.  CMS 收集器无法处理浮动垃圾。

    什么是浮动垃圾？

    CMS 在执行并发清除的时候，用户线程所产生的垃圾是不会被当前这次 GC 所处理,只好留到下一次 GC 再处理它们。这部分的垃圾就称为浮动垃圾。

    也是由于用户程序需要继续执行，所以 CMS 收集器不能像其他收集器那样等待老年代几乎都要被占满了再进行垃圾收集，而是要预留一部分内存空间供并发收集的用户程序使用。

    如果在并发收集时，用户程序找不到可分配的内存空间的时候就会导致 “Concurrent Mode Faliure”，JVM 就会启动备选方法 - Serial Old 去回收垃圾。

1.  会产生大量的空间碎片。因为 CMS 使用的是 “标记 - 清除” 算法。

    不过可以通过 `-XX:+UseCMSCompactAtFullCollection` 开关参数，在 CMS 顶不住要进行 Full GC 时进行内存的合并整理。这个时候是必须 Stop The World。

### 7.  G1（Garbage-First）收集器

![](../../imgs/jvm/jvm-14.png "G1 收集器")

G1 收集器是面向服务端应用的垃圾收集器。

G1 与 CMS 一样都是专注于减少收集所带来的停顿时间。
...
1.  初始标记（Stop The World）
    ...
    快速标记出与 GC Roots 直接相连的对象，并且修改 TAMS（Next Top at Mark Start）的值，让下一阶段用户程序并发运行时，能在正确可用的 Region 中创建对象。

1.  并发标记

    从 GC Root 开始对堆中的对象进行 **可达性分析**，找出 **存活的对象**，这个阶段耗时比较长。

1.  最终标记（Stop The World）

    修正在 **并发标记** 阶段因用户程序继续运作而导致标记产生变化的那一部分标记。

1.  筛选回收

    -   首先，对各个 Region 的回收价值和成本进行排序；

    -   接着，根据用户所期望的 GC 停顿时间来制定回收计划。

特点：

-   并行与并发

    G1 能够充分的利用多 CPU 或 多核环境下的硬件优势，来减少收集垃圾所带来的时间（Stop The World）；

    支持并发收集。

-   分代收集

    分代的概念在 G1 中还是保留的。G1 不需要其他的垃圾收集器来配合收集，因为 G1 是针对于整个 Java 堆来进行管理的，但 G1 能够采用不同的方式去处理新创建的对象（新生代的对象）、已经存活一段时间的对象（老年代的对象）和熬过多次 GC 的对象（永久代的对象）。

-   空间整理

    与 CMS 收集器采用的 “标记 - 清除” 算法不同。从整体上来看，G1 采用的是 “标记 - 整理” 算法；从局部上来看（两个 Region 之间），G1 采用的是 “复制算法”。无论是上面那种收集算法，G1 都能保证在垃圾收集之后，不会产生空间碎片。

-   可预测的收集时间

    G1 除了能够减少暂停时间之外，还能建立可预测的停顿时间时间模型，能让使用者明确指定在一个长度为 M 毫秒的时间片段内，消耗在垃圾收集上的时间不超过 N 毫秒。
