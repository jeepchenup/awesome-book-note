...
我们知道 IoC 容器最常用的就是 ApplicationContext 和 BeanFactory 这两个接口。接下来我们就以 Spring4.3.14 中的 **org.springframework.context.support.FileSystemXmlApplicationContext** 为例，来深入了解下 IoC 的初始化过程。

IoC 初始化过程分为 BeanDefinition 的 Resource 定位、加载和注册三个部分。

## 1. Resource定位
    
> 这个阶段的目的就是对 **BeanDefinition** 资源的 path 进行解析，然后生成一个**org.springframework.core.io.Resource** 对象供后面载入和解析。

看一下 FileSystemXmlApplicationContext 的继承关系以及其内部方法。

![](../../imgs/summary/sp-4-1.png)

看到 FileSystemXmlApplicationContext 中的 **getResourceByPath(String)** 是否心生疑惑，这个方法有什么用？什么时候会被调用？如果你了解过模板设计模式应该就了解其中的奥妙。

我们就从 **getResourceByPath(String)** 的方法调用栈入手。下面就跟着这张方法调用栈图来看看每个方法的实现。

![](../../imgs/summary/sp-4-2.png)

从上述的方法调用栈可以看出，**getResourceByPath(String)** 将在 **refresh()** 中被调用，同时这个方法也是 **IoC初始化** 的入口。
...
setConfigLocations(String[]) 设置 Resource 路径。
...
接下来看 **org.springframework.context.support.AbstractApplicationContext.refresh()** 方法。从上面的继承图可以看出，refresh 这个方法是实现自 **org.springframework.context.ConfigurableApplicationContext** 这个接口。
...
refresh() 方法详细的描述了 ApplicationContext 初始化的整个过程。
...
obtainFreshBeanFactory 这个方法是告诉 AbstractApplication 的子类(这里是FileSystemXmlApplicationContext)刷新内部的Bean Factory。
...
先调用了 **refreshBeanFactory()** 根据 BeanDefinition 的位置来刷新 Bean Factory。
...
loadBeanDefinitions(XmlBeanDefinitionReader) 开始加载 BeanDefinition。
...
loadBeanDefinitions - 根据 location 加载 BeanDefinition。
...
经过上面的层层调用终于到了真正加载 BeanDefinition 的方法 **org.springframework.beans.factory.support.AbstractBeanDefinitionReader.loadBeanDefinitions(String, Set)** 。
...
这里走 **else** 分支，调用 resourceLoader.getResource(location) 进行获取Resource对象。
