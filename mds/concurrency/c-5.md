...
private Node enq(final Node node) {
    for (;;) {
        ...
        // 判断尾节点是否初始化
        if (t == null) {
            // 此时说明，这是第一个进入同步队列的节点，插入同步队列之前，需要先初始化同步器（head 和 tail）
...
final boolean acquireQueued(final Node node, int arg) {
    ...
    try {
        ...
        for (;;) {
            ...
            // 获取同步状态失败之后，判断当前线程是否需要阻塞（这个是根据其前节点的等待状态来决定的）以及当前线程是否被标记被中断状态
...
1.  检查或更新前节点（相对于当前节点）的等待状态。
    
    1.  如果前节点被取消，那么及时更新当前节点的状态。

    1.  将前节点的等待状态设置为 SIGNAL。

1.  判断当前节点对应的线程是否需要被 **阻塞**。

    其前节点的等待状态为 SIGNAL 时，当前线程被阻塞，否则不需要。
...
异：
1. 节点的生成模式不同。
1. doAcquireShared 在同步队列中的节点成功获取之后，会根据其后继节点是否为 **shared** 的状态，来尝试释唤醒其后继节点。
...
2. 尝试将 head 的“后继节点”(**也有可能不是其后继节点，当一定是head的往后可遍历到的节点**)从阻塞状态中唤醒。
