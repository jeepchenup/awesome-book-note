...
-   [AQS 的代码实现](#user-content-aqs-3)
    -   [同步队列](#user-content-aqs-3-1)
    -   [独占式同步状态的获取](#user-content-aqs-3-2)
        -   [加入同步队列 - addWaiter](#user-content-aqs-3-2-1)
        -   [自旋获取同步状态 - acquireQueued](#user-content-aqs-3-2-2)
        -   [判断是否需要阻塞线程 - shouldParkAfterFailedAcquire & parkAndCheckInterrupt](#user-content-aqs-3-2-3)
    -   [独占式同步状态的释放](#user-content-aqs-3-3)
    -   [总结 - 独占式同步状态的获取与释放](#user-content-aqs-exculsive-summary)
    -   [共享式同步状态的获取](#user-content-aqs-3-4)
-   LockSupport
...
### <a id="aqs-3-2-3">shouldParkAfterFailedAcquire & parkAndCheckInterrupt</a>
...
private void unparkSuccessor(Node node) {
    ...
    // 下一个节点不为null，就将其从阻塞状态中唤醒。
    if (s != null)
        LockSupport.unpark(s.thread);
}
```

### <a id="aqs-exculsive-summary">总结 - 独占式同步状态的获取与释放</a>

就用图示对上面介绍的独占式同步状态的获取与释放做个简单的总结：

![](/imgs/concurrency/c-5-4.png)

### <a id="aqs-3-4">共享式同步状态的获取</a>
